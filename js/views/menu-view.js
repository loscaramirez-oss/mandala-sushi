...
(function (global) {
  ...
  _onSearchInput() {
    ...
    if (this.searchQuery) {
      this.nav.querySelectorAll(".chip").forEach(ch => ch.classList.remove("active"));
    }
    this.renderMenu();
  }
  ...
  renderMenu() {
    ...
    _renderSearch(main) {
      ...
      this.catalog.categories.forEach((catName, ci) => {
        const cat = this.catalog.category(ci);
        cat.items.forEach((item, ii) => {
          const hay = (item.name + " " + (item.desc || "") + " " + catName).toLowerCase();
          if (hay.indexOf(q) !== -1) results.push({ ci, ii, item });
        });
      });
      ...
      if (!results.length) {
        ...
        empty.innerHTML = '<div class="big">🍽</div>No encontramos platillos.<br>Prueba con otra palabra.';
        main.appendChild(empty);
        return;
      }
      const list = document.createElement("div");
      list.className = "items";
      results.forEach(r => list.appendChild(this._itemCard(r.ci, r.ii, r.item)));
      main.appendChild(list);
    }
    ...
    _featuredCard(ci, ii, item) {
      ...
      if (item.photo) {
        em.className = "f-photo";
        em.style.backgroundImage = "url(" + item.photo + ")";
        em.setAttribute("role", "img");
        em.setAttribute("aria-label", item.name);
      } else {
    ...
    _itemCard(ci, ii, item) {
      ...
      div.appendChild(this._iconTile(item));
    ...
    _iconTile(item) {
      if (item.photo) {
        const d = document.createElement("div");
        d.className = "item-icon photo";
        d.style.backgroundImage = "url(" + item.photo + ")";
      ...
      d.style.background = this.gradient.gradientFor(item.name);
      d.textContent = item.emoji || item.name.charAt(0);
      return d;
    }
